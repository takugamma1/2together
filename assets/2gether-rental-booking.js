...
(() => {
  ...
  function renderSummary() {
    ...
    const rDays = p.mode === 'short' ? 1 : daysIn(range.start, range.end).length; // same inclusive day count the bikes are billed for
    const qty = p.items.reduce((n, it) => n + it.qty, 0);
